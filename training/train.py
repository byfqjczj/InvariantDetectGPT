"""
Training the invariant probabilistic detector on Modal.

Input features (5 per sample):
  - per-model curvature z-scores: mistral-7b, phi-3-mini, qwen-7b
  - C_mean: mean curvature across models
  - C_var:  variance of curvature across models

Loss:
  L = L_cls + lambda_adv * L_adv + lambda_ctr * L_ctr

  L_cls: binary cross-entropy (AI vs human)
  L_adv: adversarial loss — gradient reversal on source model + domain heads
          to remove model/domain info from the learned representation
  L_ctr: contrastive loss — pull same-label samples together,
          push different-label samples apart in representation space
...
def run_training(data: list[dict]) -> dict:
    ...
    # ---------------------------------------------------------------------------
    # Split
    # Human samples are split 70/30 between train and held-out so that
    # held-out always has both classes for AUROC evaluation.
    # ---------------------------------------------------------------------------
    import random
    random.seed(42)
    ...
    # Fit label encoders on ALL sources/domains so held-out samples don't break
    ...
    # Serialize model weights as bytes to return to local machine
    import io
    buf = io.BytesIO()
...
def main():
    ...
    writeup = f"""# Detector Training Results
...
## Interpretation
The held-out AUROC of {final['held_out_auroc']:.4f} is evaluated on text from generators
...
"""
    writeup_path = os.path.join(os.path.dirname(__file__), "writeup.md")
