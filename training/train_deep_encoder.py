"""
Ablation: deeper encoder — 3 layers instead of 2.

Encoder: 5 → 128 → 64 → 32  (vs original 5 → 64 → 32)
...
app = modal.App("ai-detector-deep-encoder", image=image)
    ...
    class InvariantDetector(nn.Module):
        def __init__(self, input_dim, hidden_dim, repr_dim, n_sources, n_domains):
            ...
            self.encoder = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(hidden_dim // 2, repr_dim),
                nn.ReLU(),
            )
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "results_deep_encoder.json")
    with open(out_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {out_path}")

    model_path = os.path.join(os.path.dirname(__file__), "detector_deep_encoder.pt")
...
"""
    writeup_path = os.path.join(os.path.dirname(__file__), "writeup_deep_encoder.md")
